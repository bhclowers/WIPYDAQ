...
if __name__ == "__main__":
    ...
    fileName = 'FET_Pulser.txt'

    ans = N.loadtxt(fileName, skiprows = 0, delimiter = ',', dtype = N.float)#, usecols=(4,5))
    
    y = ans
    x = N.arange(len(y))
    
    
    _max, _min = peakdetect(y, x, 10, delta=0.01)#--IMS from FP
    _max, _min = peakdetect(y, x, 5)#for Multiplexed--TOF
    """
    Converted from/based on a MATLAB script at: 
    http://billauer.co.il/peakdet.html
    ...
    plot = pylab.plot(x, y, 'bo-', ms = 2)
    pylab.hold(True)
    pylab.plot(xm, ym, 'rd', alpha = 0.4)
    pylab.plot(xn, yn, 'gs', alpha = 0.4)
